...
p32_land_fore(t,j,ac,si,"after") =
         ...
         + sum(j_ac_land32(j,ac,land32)$(not sameas(land32,"new") AND pc32_land_fore(j,land32,si) > 0),(v32_land.l(j,land32,si)/pc32_land_fore(j,land32,si))*p32_land_fore(t,j,ac,si,"before"))$(ord(ac) > 1);

***detailed carbon stock calculations
*ccoff carbon stock = area * carbon density without CC impacts (assuming constant 1995 carbon density)
pm_carbon_stock(t,j,"forestry",c_pools,"ccoff") = sum(land32, sum(si, v32_land.l(j,land32,si))*p32_carbon_density_nocc(t,j,land32,c_pools));
*ccon carbon stock = area * carbon density including CC impacts
pm_carbon_stock(t,j,"forestry",c_pools,"ccon") = sum(land32, sum(si, v32_land.l(j,land32,si))*p32_carbon_density_cc(t,j,land32,c_pools));
