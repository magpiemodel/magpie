...
q32_establishment_min_reg(i2) ..
              ...
              =g=
              sum(kforestry, pm_demand_forestry_future(i2,kforestry) * min(0.8,sum(ct, pm_selfsuff_ext(ct,i2,kforestry)))) * sum(ct,f32_plantation_contribution(ct,i2,"%c32_dev_scen%","%c32_incr_rate%")) * (1-sum(ct,p32_fix_plant(ct)))
              ;
