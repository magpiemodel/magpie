...
parameters
 ...
 p32_replanting_cost(ac)                            Replanting costs in all age classes (USD)
 p32_time(ac)                                       Age classes as time proxy to check NPV over the growth curve (1)
 p32_discount_factor(t_all,j,ac)                        Discount factor which is a continuous time approximation for discounting (1)
 p32_forest_volume(t_all,j,ac)                          Standing forest volume (mio. m3)
 p32_net_present_value(t_all,j,ac)                      Net present value for timber plantation in each cluster depending on age-class (USD)
 p32_rotation_finder_current                        Dummy calculation for current position of rotation length finder loop (1)
 p32_rotation_finder_past                           Dummy calculation for past position of rotation length finder loop (1)
 p32_max_npv_rotation(t_all,j)                          Rotation length at which NPV is maximum for each cluster (1)
 p32_max__net_present_value
 p32_posmax_npv(t_all,j,ac)
 p32_ncells(i)
 p32_max_npv_rotation_avg(t_all,i)
 p32_land_timber_plantation(t_all,j,ac)
;
