...
i70_fac_req_livst(t_all,i,kli)$(m_year(t_all) <= sum(t_past$(ord(t_past) eq card(t_past)), m_year(t_past)) and m_year(t_all) > 1990) = (f70_hist_factor_costs_livst(t_all,i,kli) / f70_hist_prod_livst(t_all,i,kli,"dm"));
i70_fac_req_livst(t_all,i,kli)$(m_year(t_all) <= 1990) = i70_fac_req_livst("y1995",i,kli);