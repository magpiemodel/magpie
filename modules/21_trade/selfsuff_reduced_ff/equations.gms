...
*' This means that production can be freely allocated globally based on comparative advantages.
 ...
 q21_trade_forestry(kforestry)..
    ...
    sum((i2,ct),(vm_supply(i2,kforestry) + f21_trade_balanceflow(ct,kforestry)))*0.34;
* fm_production_ratio(i2,ct));

 q21_trade_natveg(kforestry)..
    sum(j2, vm_prod_natveg(j2,kforestry)) =l=
    sum((i2,ct),(vm_supply(i2,kforestry) + f21_trade_balanceflow(ct,kforestry))) * 0.66; 
* (1-fm_production_ratio(i2,ct)));
...
q21_excess_dem_ff(kforestry2)..
  ...
  =g=
  sum(i2, pc21_demand_forestry_future(i2,kforestry2)*(1 - pc21_selfsuff_forestry_future(i2,kforestry2))$(pc21_selfsuff_forestry_future(i2,kforestry2) < 1))
  + pc21_trade_balanceflow_future(kforestry2);

q21_excess_supply_ff(i2,kforestry2)..
  v21_excess_prod_ff(i2,kforestry2)
  =e=
  v21_excess_dem_ff(kforestry2)*pc21_exp_shr_future(i2,kforestry2);
