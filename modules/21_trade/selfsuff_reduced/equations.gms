...
*' This means that production can be freely allocated globally based on comparative advantages.
 ...
 q21_trade_glo(k_trade)..
  sum(h2 ,vm_prod_reg(h2,k_trade)) =g=
  sum(h2, vm_supply(h2,k_trade)) + sum(ct,f21_trade_balanceflow(ct,k_trade));
*'
*' For non-tradable commodites, the regional supply should be larger or equal to the regional demand.
 q21_notrade(h2,k_notrade)..
  vm_prod_reg(h2,k_notrade) =g= vm_supply(h2,k_notrade);
...
*' Lower bound for production.
 ...
 vm_prod_reg(h2,k_trade) =g=
 (vm_supply(h2,k_trade) + v21_excess_prod(h2,k_trade))
 *sum(ct,i21_trade_bal_reduction(ct,k_trade))
 $(sum(ct,f21_self_suff(ct,h2,k_trade) >= 1))
 + vm_supply(h2,k_trade)*sum(ct,f21_self_suff(ct,h2,k_trade))
 *sum(ct,i21_trade_bal_reduction(ct,k_trade))
 $(sum(ct,f21_self_suff(ct,h2,k_trade) < 1));
...
*' Upper bound for production.
 ...
 vm_prod_reg(h2,k_trade) =l=
 ((vm_supply(h2,k_trade) + v21_excess_prod(h2,k_trade))/sum(ct,i21_trade_bal_reduction(ct,k_trade)))
 $(sum(ct,f21_self_suff(ct,h2,k_trade) >= 1))
 + (vm_supply(h2,k_trade)*sum(ct,f21_self_suff(ct,h2,k_trade))/sum(ct,i21_trade_bal_reduction(ct,k_trade)))
 $(sum(ct,f21_self_suff(ct,h2,k_trade) < 1));
...
*' the sum over all the imports of importing regions.
 ...
 sum(h2, vm_supply(h2,k_trade)*(1 - sum(ct,f21_self_suff(ct,h2,k_trade)))
 $(sum(ct,f21_self_suff(ct,h2,k_trade)) < 1))
 ...
 *(vm_prod_reg(h2,k_trade)-vm_supply(h2,k_trade));
