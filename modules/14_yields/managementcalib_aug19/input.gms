...
table f14_yields_nogsadapt(t_all,j,kve,w) LPJmL potential yields per cell WITHOUT gsadapt (rainfed and irrigated) (tDM per ha per yr)
$ondelim
...
$if "%c14_yields_scenario%" == "nocc" f14_yields_nogsadapt(t_all,j,kve,w) = f14_yields_nogsadapt("y1995",j,kve,w);
$if "%c14_yields_scenario%" == "nocc_hist" f14_yields_nogsadapt(t_all,j,kve,w)$(m_year(t_all) > sm_fix_cc) = f14_yields_nogsadapt(t_all,j,kve,w)$(m_year(t_all) = sm_fix_cc);
m_fillmissingyears(f14_yields_nogsadapt,"j,kve,w");
