...
equations
  ...
  q15_bmi_shr_verylow(iso,sex,age_group)  estimates bmi share for population groups (capita per capita)
  q15_bmi_shr_low(iso,sex,age_group)       estimates bmi share for population groups (capita per capita)
  q15_bmi_shr_medium_high(iso,sex,age_group)     estimates bmi share for population groups (capita per capita)
  q15_bmi_shr_high(iso,sex,age_group)      estimates bmi share for population groups  (capita per capita)
  q15_bmi_shr_veryhigh(iso,sex,age_group)   estimates bmi share for population groups  (capita per capita)
  ...
  q51_bmi_shr_calib_low(iso,sex,age_group,bmi_group_est15)   estimates bmi share for population groups (capita per capita)
  q51_bmi_shr_calib(iso,sex,age_group,bmi_group_est15)   estimates bmi share for population groups (USD05MER)
  ...
  q15_regr(iso, demand_subsys15)  Share regressions  (kcal per kcal)
...
positive variables
  ...
  v15_regr(iso, demand_subsys15)       Uncalibrated regression estimates of kcal shares (-)
...
*prices
 ...
 o15_prices_kcal(t,iso,kfo)                  prices from magpie after optimization (USD05 per kcal)
 i15_prices_initial_kcal(iso,kfo)             initial prices that capture the approximate level of prices in 1961-2010 (USD05 per kcal)

* anthropometrics

  p15_bodyheight(t,iso,sex,age_group,estimates15)     body height (cm per capita)
  p15_bodyweight(t,iso,sex,age_group,bmi_group15) body weight (kg per capita)
...
parameters
 ...
 ov15_bmi_shr(t,iso,sex,age_overgroup15,bmi_group15,type)                share of population groups belonging to a certain bmi_group (capita per capita)
 ov15_bmi_shr_regr_calib(t,iso,sex,age_overgroup15,bmi_regr_type15,type) calibration parameter to meet historical BMI shares if possiblee (capita per capita)
 ov15_calib_punishment(t,iso,sex,age_overgroup15,bmi_regr_type15,type)   punishment to disinventivice but allow for divergence from calibration (USD05MER)
 ...
 oq51_bmi_shr_calib_high(t,iso,sex,age_overgroup15,bmi_regr_type15,type) estimates bmi share for population groups (capita per capita)
 oq51_bmi_shr_calib_low(t,iso,sex,age_overgroup15,bmi_regr_type15,type)  estimates bmi share for population groups (capita per capita)
 oq51_bmi_shr_calib(t,iso,sex,age_overgroup15,bmi_regr_type15,type)      estimates bmi share for population groups (USD05MER)
