...
*' the methodology established by @KleinGoldewijk.2017
 ...
 q31_manpast_suitability(i2)..
  sum(cell(i2,j2), v31_grass_area(j2,"pastr")) =l= sum((cell(i2,j2),ct),i31_manpast_suit(ct,j2));
...
q31_expansion_cost(j2) ..
  v31_cost_grass_expansion(j2) =g=
                            sum(grassland, v31_grass_area(j2, grassland) - pc31_grass(j2,grassland)) * s31_cost_expansion;

q31_cost_prod_past(i2) ..
  vm_cost_prod(i2,"pasture") =e= sum((cell(i2,j2), grassland), v31_grass_area(j2, grassland) * v31_grass_yld(j2, grassland)) * s31_cost_grass_prod +
                                 sum(cell(i2,j2), v31_cost_grass_expansion(j2));
