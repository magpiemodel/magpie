...
*' @code
*' In MAgPIE we can assume to only model 50 percent of the projected woodfuel demand.
...
*' production, We make an assumption similar to IMAGE.

fm_forestry_demand(t_all,i,kforestry) = f16_forestry_demand(t_all,i,kforestry);

*' While fuelwood production in industrialized regions is dominated by large-scale, commercial operations,
*' in transitional and developing regions smaller proportions of fuelwood volumes are assumed to come from
*' forestry operations: 50% and 32% respectively.
...
*' Following parameters are displayed in the lst file for quick glamce.
