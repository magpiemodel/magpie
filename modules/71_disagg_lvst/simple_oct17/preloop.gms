...
i71_share_in_feedmix_reg(t_all,i,kli,kall,dm_ge_nr) =
	   ...
	   sum( kall,im_feed_baskets(t_all,i,kli,kall) * fm_attributes(dm_ge_nr,kall) )
	   ;
	   
i71_share_in_feedmix_reg(t_all,j,kli,kall,dm_ge_nr) =	   
	   sum(cell(i,j),i71_share_in_feedmix_reg(t_all,i,kli,kall,dm_ge_nr))
	   ;
	   
i71_urban_area_share(j) =
       pm_land_start(j,"urban") / sum(cell(i,j),sum(cell(i,j),pm_land_start(j,"urban"))) 
	   ;