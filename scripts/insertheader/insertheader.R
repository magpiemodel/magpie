...
insertheader <- function(maindir="../../", header="scripts/insertheader/header.lhd", oldheader="scripts/insertheader/oldheader.lhd", line_endings="notwin") {
  ...
  .findheader <- function(h,f){
    ...
    # For all matches of first line of header: look for matches for next line of header
    # Only if next line of header matches next line in file, mark line number and continue
	# Else do not continue search for next elemens of h 
    for (before in first) {
      for (i in 2:length(h)) {
    ...
    # Return only full matches (list elements with number of elements equal to number of lines in header)
  ...
  donottouch <- c("AUTHORS","README","LICENSE",".lhd",".mz",".rda",".opt",".spam",".xlsx",".sh",".csv")
  
  comments <- c(".R"="#",".gms"="***",".cfg"="#",".cs2"="*",".cs3"="*",".cs4"="*",".sh"="#",".txt"="#")
