...
ncatt_put(ncnew,0,"contact","F. Humpenöder humpenoeder@pik-potsdam.de")
...
plotmap2(landOutShr[, 2050, "rest"],legend_range = c(0,1))

plotmap2(dimSums(landOutShr, dim = 3)[, 2050,],legend_range = c(0,1))


# a <- readGDX(gdx, "ov_lu_transitions",  select = list(type = "level"), react = "silent")
# b <- dimSums(a[,,c("primforest.urban","secdforest.urban","other.urban")], c(1,3))

# d <- dimSums(landRestorHr, c(1,3))

# print(round(b+d,2))

# print(round(dimSums(.calcDiffLand(forestHr), c(1,3)),2))

# otherLandLr <- magpie4::OtherLand(gdx, level = "cell")

# cropLr <- magpie4::land(gdx, level = "cell", types = "crop")
# dCrop <- -.calcDiffLand(cropLr)
# dCrop[dCrop < 0] <- 0


# # grassland at cluster level
# grassLr <- magpie4::land(gdx, level = "cell", types = "past")
# careaShr

# dOtherLand <- .calcDiffLand(dimSums(otherLandLr[,,c("other_recovered")],dim=3))
# dOtherLand[dOtherLand < 0] <- 0
# print(dimSums(dOtherLand, dim = c(1,3), na.rm = TRUE))
# print(dimSums((dCrop+dgrass),dim=c(1,3)))

# print(dimSums(dOtherLand, dim = c(1,3), na.rm = TRUE))
# print(dimSums(dcropOther, dim = c(1,3), na.rm = TRUE))
# print(dimSums(dcropBE, dim = c(1,3), na.rm = TRUE))
# print(dimSums(dgrass, dim = c(1,3), na.rm = TRUE))

# print(dimSums(dCrop, dim = c(1,3), na.rm = TRUE))

# # derive reduction of cropland and grassland pools over time
# dcropOther <- -.calcDiffLand(cropOtherLr)
# dcropOther[dcropOther < 0] <- 0

# dcropBE <- -.calcDiffLand(cropBELr)
# dcropBE[dcropBE < 0] <- 0

# dgrass <- -.calcDiffLand(grassLr)
# dgrass[dgrass < 0] <- 0

# summary(dimSums((dCrop+dgrass),dim=c(1,3))/dimSums(dOtherLand, dim = c(1,3)))
# print(dimSums((dCrop+dgrass),dim=c(1,3))/dimSums(dOtherLand, dim = c(1,3)))


# # calculate share of recovered other land in total other land
# otherLandRecShr <- otherLandLr[, , "other_recovered"] / dimSums(otherLandLr[, , c("other_initial", "other_recovered")], dim = 3)
# otherLandRecShr[is.na(otherLandRecShr)] <- 0



# # other land expansion due to abandonment of cropland and grassland
# # restoration area has already been subtracted from other land
# dOtherLand <- -.calcDiffLand(landUnmgdShr[, , "other"])
# dOtherLand[dOtherLand < 0] <- 0
...
# # derive reduction of cropland and grassland pools over time
# dcropOther <- -.calcDiffLand(cropOtherShr)
# dcropOther[dcropOther < 0] <- 0

# dcropBE <- -.calcDiffLand(cropBEShr)
# dcropBE[dcropBE < 0] <- 0

# dgrass <- -.calcDiffLand(grassShr)
# dgrass[dgrass < 0] <- 0


# summary((dcropOther+dcropBE+dgrass)/dOtherLand)
...
# otherShr <- setNames(landUnmgdShr[, , c("other")], "other")
