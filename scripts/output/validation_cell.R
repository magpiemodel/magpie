...
LU <- plotCorrHist2D(x = historical_LU, y = magpie_LU, xlab = "LUH2 \n (mio.ha)", ylab = "MAgPIE \n (mio.ha)", bins = 35,
...
swlatex(sw, "\\section{Land use between 1995-2020}")
...
swtable(sw, collapseNames(as.magpie(y[a:b, ])), table.placement = "H", caption.placement = "top", 
...
swlatex(sw, "\\section{Crop types 1995-2015 LUH}")
...
swlatex(sw, "\\newpage")
swlatex(sw, "\\section{Crop types 2000-2010 - MAPSPAM and LUH comparison}")
...
########## Details about MAEH ##########
swlatex(sw, "\\newpage")
swlatex(sw, "*MAEH Stands for Mean Absolute Error per hectar. It is calculated as $MAE=\\sum(|error|)/area$ . Here, error represents the difference between LUH/MAPSPAM values and MAgPIE Outputs.")
swlatex(sw, " Area is the overal world area for the specific land use or crop type in time t as reported by LUH2 or MAPSPAM")
...
fileErase <- c(Sys.glob(file.path(outputdir, "/CellValidation.*")),
