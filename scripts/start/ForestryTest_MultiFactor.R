...
for(factor_multiplier in c(100,1000,10000,100000)){

	cfg$gms$s32_recurring_cost_multiplier <- factor_multiplier
			...
			for(co2_price_scenarios in c("R2M41-SSP2-NPi")) {
				...
				for (sl_set in c(1.00)) {
					...
					if(cfg$gms$c56_pollutant_prices == "R2M41-SSP2-Budg1300" ) {
						...
						cfg$title<- paste0(flag_run,"-","Factor",factor_multiplier)
					}
