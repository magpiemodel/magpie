...
prefix <- paste(rev, "SNVTests_CurrentDev", sep = "_")

scenarios <- c(
  "default", "SNV10","SNV20","SNV30","SNV50"
)
  ...
  for (scen in scenarios) {
    ...
    # permanent habitats
    if (any(actions %in% c("SNV10"))) {
      cfg$gms$s30_snv_shr <- 0.1
      cfg$gms$s30_snv_scenario_target <- 2030
    }
    ...
    if (any(actions %in% c("SNV30"))) {
      cfg$gms$s30_snv_shr <- 0.3
      cfg$gms$s30_snv_scenario_target <- 2030
    }

    if (any(actions %in% c("SNV50"))) {
      cfg$gms$s30_snv_shr <- 0.5
      cfg$gms$s30_snv_scenario_target <- 2030
    }
