...
calc_bioen <- function(x) {
  ...
  B50 <- new.magpie("GLO",c(seq(1995,2010,by=5),2050,2100,2150),NULL,fill = 0)
  B50[,"y2050",] <- 50*1000
  B50[,"y2100",] <- 112.5*1000
  B50[,"y2150",] <- 112.5*1000
  B50 <- time_interpolate(B50,seq(2010,2050,by=5),TRUE)
...
calc_ghgprice <- function(x) {
  ...
  T100 <- new.magpie(getRegions(T0),c(seq(1995,2015,by=5),2100,2150),getNames(T0),fill = 0)
  T100[,"y2015","co2_c"] <- 25
  T100[,"y2100","co2_c"] <- 450
  T100[,"y2150","co2_c"] <- 450
  T100 <- time_interpolate(T100,seq(2015,2100,by=5),TRUE)
  T100 <- time_interpolate(T100,seq(2100,2150,by=5),TRUE)
  T100[,,"ch4"] <- T100[,,"co2_c"]*28
  T100[,,"n2o_n_direct"] <- T100[,,"co2_c"]*265*44/28
  T100[,,"n2o_n_indirect"] <- T100[,,"co2_c"]*265*44/28
  T100[,,"co2_c"] <- T100[,,"co2_c"]*44/12
      ...
      for (ghgprice in c("T0","T40","T100")) {
        for (bioen_type in c("all","begr","betr")) {
          for (bioen_water in c("all","rainfed")) {
            ...
            cfg$gms$c30_bioen_water <- bioen_water
            download_and_update(cfg)
            write.magpie(readGDX(file.path("output",paste("TAU",ssp,tau_scen,sep="-"),"fulldata.gdx"), "ov_tau", select=list(type="level")),"modules/13_tc/input/f13_tau_scenario.csv")
            write.magpie(calc_bioen(biodem),"modules/60_bioenergy/input/glo.2ndgen_bioenergy_demand.csv")
            write.magpie(calc_ghgprice(ghgprice),"modules/56_ghg_policy/input/f56_pollutant_prices_emulator.cs3")
            start_run(cfg,codeCheck=FALSE)
          }
