...
## Create empty list to check titles
xx <- c()
...
for(cc in c("nocc","cc")){
  for(ssp in c("SSP2")) { ## Add SSP* here for testing other SSPs. Basic test should be for SSP2
    for(macc in c("PBL_2019")) {
      for (co2_price_path in c("Baseline")) {
  #    for (co2_price_path in c("Baseline","Policy")) {
        ...
        cfg$title <- paste0(pr_flag,"_",user,"_",ssp,"_",co2_price_path,"_",cfg$gms$c14_yields_scenario) #Create easily distinguishable run title
