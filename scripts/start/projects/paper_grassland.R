...
# ------------------------------------------------
# description:Runs for grassland management paper. Should be replaced by updating
# setScenario.csv with paramerts for cfg$gms$c31_past_suit_scen in the future.
# ------------------------------------------------
library(gms)
...
for(ssp_setting in c("SSP1","SSP2","SSP3","SSP4","SSP5")){
  ...
  cfg$title <- paste0("IPSL-CM6A-LR-",ssp_setting,"-set1-", substr(Sys.time(), 6,10),"-",gsub(":", "_", substr(Sys.time(), 12,16)))
