  ...
  runsubmit <- function(output, outputdirs, comp, script_path) {
    ...
    for(rout in output){
      ...
      if(submit=="direct") {
        ...
        system(paste0("srun --qos=short --job-name=scripts-output --output=log_out-%j.out --error=log_out-%j.err --mail-type=END --time=200 --mem-per-cpu=6000 Rscript output.R outputdirs=",paste(outputdirs,collapse=",")," comp=",comp,"  output=",rout," submit=direct &"))
        Sys.sleep(1)
