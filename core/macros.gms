...
$macro m_carbon_stock(vm_land,fm_carbon_density,item) \
            (vm_land(j2,item) * sum(ct,fm_carbon_density(ct,j2,item,ag_pools)))$(sameas(stockType,"actual")) + \
            (vm_land(j2,item) * sum(ct,fm_carbon_density(ct,j2,item,ag_pools)))$(sameas(stockType,"actualNoAcEst")) + \
            (vm_land.l(j2,item) * sum(ct,fm_carbon_density(ct,j2,item,ag_pools)))$(sameas(stockType,"previousLandPattern")) + \
            (vm_land(j2,item) * sum(pt,fm_carbon_density(pt,j2,item,ag_pools)))$(sameas(stockType,"previousCarbonDensity"));

* macro for carbon stocks with age classes
$macro m_carbon_stock_ac(v32_land,p32_carbon_density_ac,sets,sets_sub) \
            sum((&&sets), v32_land(j2,&&sets) * sum(ct, p32_carbon_density_ac(ct,j2,&&sets,ag_pools)))$(sameas(stockType,"actual")) + \
            sum((&&sets_sub), v32_land(j2,&&sets_sub) * sum(ct, p32_carbon_density_ac(ct,j2,&&sets_sub,ag_pools)))$(sameas(stockType,"actualNoAcEst")) + \
            sum((&&sets), v32_land.l(j2,&&sets) * sum(ct, p32_carbon_density_ac(ct,j2,&&sets,ag_pools)))$(sameas(stockType,"previousLandPattern")) + \
            sum((&&sets), v32_land(j2,&&sets) * sum(pt, p32_carbon_density_ac(pt,j2,&&sets,ag_pools)))$(sameas(stockType,"previousCarbonDensity"));
