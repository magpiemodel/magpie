...
- **30_croparea** `penalties_sep26` restricts the rotation rule set to the rules that carry a penalty and uses a single fixed set of thresholds instead of the `c30_rotation_rules` scenarios. Minimum-share rules are enforced for the first time. This changes results relative to `detail_apr24` and requires recalibration.
